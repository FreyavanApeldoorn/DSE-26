...
class Mass_Est:
    ...
    def plot(self):
        ...
        plt.plot(self.W_S, self.P_W_cruise, label = "Cruise P_W")
        plt.plot(self.W_S, self.P_W_climb, label = "Climb P_W")
        plt.plot(self.W_S, self.P_W_service, label = "Serivice P_W")
        #plt.plot(W_S, W_S_stall, label = "Stall Speed W_S")
        plt.vlines(self.WingLoading_Vstall(), 0,100, label= "Stall Speed W_S" )
